...
// 💡 1. 在組件最上方定義隨機提示語 (約第 135 行)
  const FOOTER_HINTS = [
    ...
    ["💡 右上角「生成桌布」，快速查看團序", "🔔 遮罩功能 3/21 啟用"],
    ["💡 點擊成員框框對比團序", "🔔 遮罩功能 3/21 啟用"],
    ["💡 點擊右上角COLOR，設定代表顏色", "🔔 遮罩功能 3/21 啟用"],
    ["💡 人氣依同時段勾選比例計算，建議即早準備", "🔔 遮罩功能 3/21 啟用"]
  ];
...
// 💡 修改這段原本偵測網路的 useEffect
  useEffect(() => {
    const handleOnline = () => {
      console.log("🌐 偵測到網路恢復！啟動暴力同步...");
      // ✅ 改成執行上傳同步，而不是下載覆蓋
      syncLocalToCloud(); 
    };
  ...
  }, [allSelections, currentSquad]); // 💡 加入 allSelections 作為依賴，確保同步時拿到的是最新的狀態
  // 💡 定時每分鐘更新一次時間
  useEffect(() => {
...
useEffect(() => {
    if (mounted) {
      localStorage.setItem('megaport_offline_selections', JSON.stringify(allSelections));
    }
  }, [allSelections, mounted]);
